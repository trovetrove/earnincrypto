...
export function NFTToolForm({ entry }: { entry?: CryptoEntry }) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      <input type="hidden" name="category" value="nft-tools" />
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Tool Name" error={err("title")} required><Input name="title" value={f.title} onChange={str("title")} placeholder='"OpenSea"' className={IC} required /></F>
          <F label="Slug" error={err("slug")} required><Input name="slug" value={f.slug} onChange={str("slug")} placeholder="opensea" className={IC_MONO} required /></F>
          <F label="One-liner" error={err("shortDescription")} required><Input name="shortDescription" value={f.shortDesc} onChange={str("shortDesc")} placeholder="The largest NFT marketplace with multi-chain support" className={IC} required /></F>
          <F label="Key Highlight"><Input name="potential" value={f.potential} onChange={str("potential")} placeholder="0% listing fees, cross-chain trading" className={IC} /></F>
        </div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">🎨 Platform Details</h3>
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <F label="Platform Token"><Input name="token" value={f.token} onChange={str("token")} placeholder="" className={IC} /></F>
          <F label="Pricing">
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        ...
        <F label="Full Description" error={err("description")} required><RichEditor name="description" defaultValue={entry?.description} placeholder="Describe: minting, marketplace, analytics features. Use H2 for 'Features', 'Fees', 'Supported NFTs'." minHeight={220} /></F>
        <div className="mt-4"><F label="Free Tier / Details"><Input name="freeTierDetails" value={f.freeTierDetails} onChange={str("freeTierDetails")} placeholder="0% listing. 2.5% sale fee. Creator royalties vary." className={IC} /></F></div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">✅ Pros & Cons</h3>
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Pros (one per line)"><Textarea name="pros" value={f.pros} onChange={str("pros")} rows={4} placeholder={"Largest marketplace\nMulti-chain\nEasy to use\nVerification system"} className={TC} /></F>
          <F label="Cons (one per line)"><Textarea name="cons" value={f.cons} onChange={str("cons")} rows={4} placeholder={"High gas on ETH\nFake collections\nSlow support\nRoyalty issues"} className={TC} /></F>
        </div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">🚀 How to Get Started</h3>
        <F label="Steps (one per line)"><Textarea name="howToUse" value={f.howToUse} onChange={str("howToUse")} rows={5} placeholder={"Connect wallet\nBrowse collections\nBuy or make offer\nList your NFTs"} className={TC} /></F>
      </div>
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Tags"><Input name="tags" value={f.tags} onChange={str("tags")} placeholder="nft, marketplace, art, collectibles" className={IC} /></F>
          <F label="Target Audience"><Input name="audience" value={f.audience} onChange={str("audience")} placeholder="NFT collectors, Artists, Traders" className={IC} /></F>
          <F label="Best For"><Input name="bestFor" value={f.bestFor} onChange={str("bestFor")} placeholder="Users wanting to buy/sell NFTs easily" className={IC} /></F>
          <F label="Alternatives"><Input name="alternatives" value={f.alternatives} onChange={str("alternatives")} placeholder="Blur, Magic Eden, Foundation" className={IC} /></F>
      ...
      <div className="flex items-center gap-3">
        <button type="submit" disabled={isPending} className="flex items-center gap-2 bg-[#7C4DFF] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#7C4DFF]/80 disabled:opacity-50" style={{ border: "2px solid rgba(255,255,255,0.1)" }}>{isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{entry ? "Update NFT Tool" : "Create NFT Tool"}</button>
