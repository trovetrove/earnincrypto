...
export function InfrastructureForm({ entry }: { entry?: CryptoEntry }) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      <input type="hidden" name="category" value="infrastructure" />
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">⛓️ Service Identity</h3>
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Service Name" error={err("title")} required><Input name="title" value={f.title} onChange={str("title")} placeholder='"Alchemy"' className={IC} required /></F>
          <F label="Slug" error={err("slug")} required><Input name="slug" value={f.slug} onChange={str("slug")} placeholder="alchemy" className={IC_MONO} required /></F>
          <F label="One-liner" error={err("shortDescription")} required><Input name="shortDescription" value={f.shortDesc} onChange={str("shortDesc")} placeholder="Enterprise-grade RPC and node infrastructure for Web3" className={IC} required /></F>
          <F label="Key Highlight"><Input name="potential" value={f.potential} onChange={str("potential")} placeholder="300M compute units/mo free tier" className={IC} /></F>
        </div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">🔧 Technical Details</h3>
        <div className="grid gap-4 md:grid-cols-3">
          <F label="Supported Chains"><Input name="chain" value={f.chain} onChange={str("chain")} placeholder="Ethereum, Polygon, Solana" className={IC} /></F>
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">📋 Service Overview</h3>
        <F label="Full Description" error={err("description")} required><RichEditor name="description" defaultValue={entry?.description} placeholder="Describe: RPC capabilities, APIs, SDKs, uptime SLA. Use H2 for 'Features', 'API Access', 'Free Tier'." minHeight={220} /></F>
        <div className="mt-4"><F label="Free Tier / Details"><Input name="freeTierDetails" value={f.freeTierDetails} onChange={str("freeTierDetails")} placeholder="300M compute units/mo, 5 apps, enhanced APIs." className={IC} /></F></div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">✅ Pros & Cons</h3>
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Pros (one per line)"><Textarea name="pros" value={f.pros} onChange={str("pros")} rows={4} placeholder={"Generous free tier\nGreat docs\nMulti-chain\nWebhooks\nAnalytics"} className={TC} /></F>
          <F label="Cons (one per line)"><Textarea name="cons" value={f.cons} onChange={str("cons")} rows={4} placeholder={"Rate limits on free\nVendor lock-in\nExpensive at scale\nSome chains beta"} className={TC} /></F>
        </div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">🚀 How to Get Started</h3>
        <F label="Steps (one per line)"><Textarea name="howToUse" value={f.howToUse} onChange={str("howToUse")} rows={5} placeholder={"Sign up free\nCreate project\nGet API key\nReplace RPC endpoint\nMonitor dashboard"} className={TC} /></F>
      </div>
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Tags"><Input name="tags" value={f.tags} onChange={str("tags")} placeholder="rpc, node, api, infrastructure" className={IC} /></F>
          <F label="Target Audience"><Input name="audience" value={f.audience} onChange={str("audience")} placeholder="Web3 developers, dApp builders" className={IC} /></F>
          <F label="Best For"><Input name="bestFor" value={f.bestFor} onChange={str("bestFor")} placeholder="Developers needing reliable blockchain infrastructure" className={IC} /></F>
          <F label="Alternatives"><Input name="alternatives" value={f.alternatives} onChange={str("alternatives")} placeholder="Infura, QuickNode, Ankr" className={IC} /></F>
      ...
      <div className="flex items-center gap-3">
        <button type="submit" disabled={isPending} className="flex items-center gap-2 bg-[#7C4DFF] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#7C4DFF]/80 disabled:opacity-50" style={{ border: "2px solid rgba(255,255,255,0.1)" }}>{isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{entry ? "Update Service" : "Create Service"}</button>
