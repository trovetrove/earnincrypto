...
export function LaunchpadForm({ entry }: { entry?: CryptoEntry }) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      <input type="hidden" name="category" value="launchpads" />
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">🚀 Platform Identity</h3>
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Platform Name" error={err("title")} required><Input name="title" value={f.title} onChange={str("title")} placeholder='"DAO Maker"' className={IC} required /></F>
          <F label="Slug" error={err("slug")} required><Input name="slug" value={f.slug} onChange={str("slug")} placeholder="dao-maker" className={IC_MONO} required /></F>
          <F label="One-liner" error={err("shortDescription")} required><Input name="shortDescription" value={f.shortDesc} onChange={str("shortDesc")} placeholder="Token launchpad with community-backed vetting and staking" className={IC} required /></F>
          <F label="Key Highlight"><Input name="potential" value={f.potential} onChange={str("potential")} placeholder="10x–100x historical average" className={IC} /></F>
        </div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">🎯 Launch Details</h3>
        <div className="grid gap-4 md:grid-cols-3">
          <F label="Primary Chain"><Input name="chain" value={f.chain} onChange={str("chain")} placeholder="Ethereum, BSC, Polygon" className={IC} /></F>
          <F label="Platform Token"><Input name="token" value={f.token} onChange={str("token")} placeholder="DAO" className={IC} /></F>
          <F label="Participation Cost">
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">📋 Platform Overview</h3>
        <F label="Full Description" error={err("description")} required><RichEditor name="description" defaultValue={entry?.description} placeholder="Describe: sale types (IDO/IEO), vetting, past performance, staking tiers. Use H2 for 'How It Works', 'Past Projects', 'Tiers'." minHeight={220} /></F>
        <div className="mt-4"><F label="Free Tier / Details"><Input name="freeTierDetails" value={f.freeTierDetails} onChange={str("freeTierDetails")} placeholder="Stake 500 DAO minimum. Higher tiers: guaranteed allocation." className={IC} /></F></div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">✅ Pros & Cons</h3>
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Pros (one per line)"><Textarea name="pros" value={f.pros} onChange={str("pros")} rows={4} placeholder={"Strong track record\nCommunity vetting\nMultiple tiers\nRefund policy\nMulti-chain"} className={TC} /></F>
          <F label="Cons (one per line)"><Textarea name="cons" value={f.cons} onChange={str("cons")} rows={4} placeholder={"Requires staking\nNot all succeed\nHigh competition\nLock-ups\nPast ≠ future"} className={TC} /></F>
        </div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">🚀 How to Participate</h3>
        <F label="Steps (one per line)"><Textarea name="howToUse" value={f.howToUse} onChange={str("howToUse")} rows={5} placeholder={"Sign up and KYC\nBuy and stake tokens\nBrowse launches\nApply for allocation\nDeposit and receive tokens"} className={TC} /></F>
      </div>
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Tags"><Input name="tags" value={f.tags} onChange={str("tags")} placeholder="launchpad, ido, ieo, token sale" className={IC} /></F>
          <F label="Target Audience"><Input name="audience" value={f.audience} onChange={str("audience")} placeholder="Investors, Early adopters, Stakers" className={IC} /></F>
          <F label="Best For"><Input name="bestFor" value={f.bestFor} onChange={str("bestFor")} placeholder="Investors wanting early access to new tokens" className={IC} /></F>
          <F label="Alternatives"><Input name="alternatives" value={f.alternatives} onChange={str("alternatives")} placeholder="Seedify, Polkastarter, TrustPad" className={IC} /></F>
      ...
      <div className="flex items-center gap-3">
        <button type="submit" disabled={isPending} className="flex items-center gap-2 bg-[#7C4DFF] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#7C4DFF]/80 disabled:opacity-50" style={{ border: "2px solid rgba(255,255,255,0.1)" }}>{isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{entry ? "Update Launchpad" : "Create Launchpad"}</button>
