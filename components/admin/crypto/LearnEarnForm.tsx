...
export function LearnEarnForm({ entry }: { entry?: CryptoEntry }) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      <input type="hidden" name="category" value="learn-earn" />
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">📚 Program Identity</h3>
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Program Name" error={err("title")} required><Input name="title" value={f.title} onChange={str("title")} placeholder='"Coinbase Learn"' className={IC} required /></F>
          <F label="Slug" error={err("slug")} required><Input name="slug" value={f.slug} onChange={str("slug")} placeholder="coinbase-learn" className={IC_MONO} required /></F>
          <F label="One-liner" error={err("shortDescription")} required><Input name="shortDescription" value={f.shortDesc} onChange={str("shortDesc")} placeholder="Earn crypto by watching videos and completing quizzes" className={IC} required /></F>
          <F label="Key Highlight"><Input name="potential" value={f.potential} onChange={str("potential")} placeholder="$5–$30 in various tokens" className={IC} /></F>
        </div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">🎓 Program Details</h3>
        <div className="grid gap-4 md:grid-cols-3">
          <F label="Platform"><Input name="chain" value={f.chain} onChange={str("chain")} placeholder="Coinbase (Multi-chain)" className={IC} /></F>
          <F label="Reward Token"><Input name="token" value={f.token} onChange={str("token")} placeholder="Various" className={IC} /></F>
          <F label="Cost">
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">📋 Program Overview</h3>
        <F label="Full Description" error={err("description")} required><RichEditor name="description" defaultValue={entry?.description} placeholder="Describe: courses, reward structure, payout method, eligibility. Use H2 for 'Courses', 'Rewards', 'Payouts'." minHeight={220} /></F>
        <div className="mt-4"><F label="Free Tier / Details"><Input name="freeTierDetails" value={f.freeTierDetails} onChange={str("freeTierDetails")} placeholder="Instant payout to wallet. Must complete quiz correctly." className={IC} /></F></div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">✅ What You Get & Limitations</h3>
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Pros (one per line)"><Textarea name="pros" value={f.pros} onChange={str("pros")} rows={4} placeholder={"Free crypto\nNo investment needed\nLearn about projects\nInstant payouts\nWorldwide"} className={TC} /></F>
          <F label="Cons (one per line)"><Textarea name="cons" value={f.cons} onChange={str("cons")} rows={4} placeholder={"Limited per course ($1-5)\nWaitlist for new courses\nKYC required\nSome geo-restricted"} className={TC} /></F>
        </div>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">🚀 How to Earn</h3>
        <F label="Steps (one per line)"><Textarea name="howToUse" value={f.howToUse} onChange={str("howToUse")} rows={5} placeholder={"Sign up and verify\nBrowse courses\nWatch video\nComplete quiz\nReceive rewards"} className={TC} /></F>
      </div>

      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-white/30">💬 Is It Worth Your Time?</h3>
      ...
      <div className="border border-white/[0.06] bg-white/[0.03] p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <F label="Tags"><Input name="tags" value={f.tags} onChange={str("tags")} placeholder="learn, earn, education, rewards" className={IC} /></F>
          <F label="Target Audience"><Input name="audience" value={f.audience} onChange={str("audience")} placeholder="Crypto beginners, Students" className={IC} /></F>
          <F label="Best For"><Input name="bestFor" value={f.bestFor} onChange={str("bestFor")} placeholder="People wanting free crypto while learning" className={IC} /></F>
          <F label="Alternatives"><Input name="alternatives" value={f.alternatives} onChange={str("alternatives")} placeholder="CoinMarketCap Earn, Revolut Learn" className={IC} /></F>
      ...
      <div className="flex items-center gap-3">
        <button type="submit" disabled={isPending} className="flex items-center gap-2 bg-[#7C4DFF] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#7C4DFF]/80 disabled:opacity-50" style={{ border: "2px solid rgba(255,255,255,0.1)" }}>{isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{entry ? "Update Program" : "Create Program"}</button>
