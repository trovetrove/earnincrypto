...
export async function getAllCryptoEntries(): Promise<CryptoEntry[]> {
  ...
  const { data, error } = await sb
    .from("crypto_entries").select("*").order("created_at", { ascending: false });
  if (error) { console.error("[getAllCryptoEntries]", error.message); return []; }
...
export async function getCryptoEntryBySlug(slug: string): Promise<CryptoEntry | null> {
  const sb = getSupabaseServer();
  const { data, error } = await sb
    .from("crypto_entries").select("*").eq("slug", slug).single();
  if (error) {
    if (error.code !== "PGRST116") console.error("[getCryptoEntryBySlug]", error.message);
    return null;
  }
  return mapRow(data);
}
...
export async function getCryptoEntriesByCategory(categorySlug: string): Promise<CryptoEntry[]> {
  const sb = getSupabaseServer();
  const { data, error } = await sb
    .from("crypto_entries").select("*").eq("category", categorySlug)
    .order("is_featured", { ascending: false }).order("rating", { ascending: false });
  if (error) { console.error("[getCryptoEntriesByCategory]", error.message); return []; }
  return (data as any[]).map(mapRow);
}
...
export async function getFeaturedCryptoEntries(limit = 8): Promise<CryptoEntry[]> {
  const sb = getSupabaseServer();
  const { data, error } = await sb
    .from("crypto_entries").select("*").eq("is_featured", true)
    .order("rating", { ascending: false }).limit(limit);
  if (error) { console.error("[getFeaturedCryptoEntries]", error.message); return []; }
  return (data as any[]).map(mapRow);
}

export async function getRelatedCryptoEntries(
  categorySlug: string, excludeId: string, limit = 4
): Promise<CryptoEntry[]> {
  const sb = getSupabaseServer();
  const { data, error } = await sb
    .from("crypto_entries").select("*").eq("category", categorySlug)
    .neq("id", excludeId).order("rating", { ascending: false }).limit(limit);
  if (error) { console.error("[getRelatedCryptoEntries]", error.message); return []; }
  return (data as any[]).map(mapRow);
}
...
export async function getGlobalRandomCrypto(excludeId: string, limit = 6): Promise<CryptoEntry[]> {
  const sb = getSupabaseServer();
  const { data, error } = await sb
    .from("crypto_entries").select("*").neq("id", excludeId)
...
export async function getCryptoStats() {
  ...
  const { count: totalTools } = await sb
    .from("crypto_entries").select("*", { count: "exact", head: true });
  const { count: featuredCount } = await sb
    .from("crypto_entries").select("*", { count: "exact", head: true }).eq("is_featured", true);
  const { count: airdropCount } = await sb
    .from("crypto_entries").select("*", { count: "exact", head: true }).eq("category", "airdrops");
  return {
