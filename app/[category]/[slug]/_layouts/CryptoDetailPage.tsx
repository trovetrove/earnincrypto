 ...
 * Dark-themed detail page for crypto entries.
 * Adapts sections based on category (airdrop vs exchange vs DeFi etc.)
 */
...
import {
  ...
  ChevronDown, Shield, Smartphone, Sparkles, AlertTriangle,
  Zap, Clock, Target, Gift, TrendingUp,
} from "lucide-react";
...
// Category-specific section labels
function getSectionLabels(category: string) {
  switch (category) {
    case "airdrops":
      return { desc: "About This Airdrop", pros: "Why Eligible", cons: "Requirements & Risks", steps: "How to Claim", verdict: "Is This Worth It?" };
    case "exchanges":
      return { desc: "Exchange Overview", pros: "Strengths", cons: "Weaknesses", steps: "How to Get Started", verdict: "Our Verdict" };
    case "defi-yield":
      return { desc: "Protocol Overview", pros: "Advantages", cons: "Risks", steps: "How to Start Earning", verdict: "Realistic Yields" };
    case "wallets":
      return { desc: "Wallet Overview", pros: "Pros", cons: "Cons", steps: "Setup Guide", verdict: "Our Take" };
    case "learn-earn":
      return { desc: "Program Overview", pros: "What You Get", cons: "Limitations", steps: "How to Earn", verdict: "Is It Worth Your Time?" };
    default:
      return { desc: `About ${category}`, pros: "Pros", cons: "Cons", steps: "How to Use", verdict: "Our Verdict" };
  }
}
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              <div className="flex-1">
                <div className="mb-3 flex flex-wrap gap-2">
                  <span className="px-2 py-0.5 text-[10px] font-bold uppercase" style={{ background: price.bg, color: price.color }}>
                    {price.label}
                  </span>
                  <span className="px-2 py-0.5 text-[10px] font-bold uppercase" style={{ background: risk.bg, color: risk.color }}>
                    {risk.label}
                  </span>
                  {entry.chain && (
                    <span className="border border-white/10 px-2 py-0.5 text-[10px] font-medium text-white/40">
                      {entry.chain}
                    </span>
                  )}
                  {entry.token && (
                    <span className="border border-[#7C4DFF]/30 bg-[#7C4DFF]/10 px-2 py-0.5 text-[10px] font-bold text-[#7C4DFF]">
                      ${entry.token}
                    </span>
                  )}
                  {entry.isFeatured && (
                    <span className="bg-[#F5C842] px-2 py-0.5 text-[10px] font-bold text-[#0a0a0a]">
                      <Sparkles className="mr-1 inline h-2.5 w-2.5" />Featured
                    </span>
                  )}
                  {entry.isVerified && (
                    <span className="bg-[#34D163] px-2 py-0.5 text-[10px] font-bold text-[#0a0a0a]">
                      <Shield className="mr-1 inline h-2.5 w-2.5" />Verified
                    </span>
                  )}
                </div>
              ...
              <div className="shrink-0 lg:w-72">
                <div className="border border-[#7C4DFF]/30 bg-[#7C4DFF]/5 p-6" style={{ boxShadow: "4px 4px 0 #7C4DFF50" }}>
                  ...
                  {entry.freeTierDetails && (
                    <p className="mb-4 text-xs text-white/30">{entry.freeTierDetails}</p>
                  )}
            ...
            <div className="grid gap-8 lg:grid-cols-3">
              <div className="space-y-6 lg:col-span-2">
                ...
                {globalPicks.length > 0 && (
                  <div className="border-t border-white/[0.06] pt-8">
                    <h2 className="mb-4 font-display text-sm font-bold uppercase tracking-widest text-white/40">You May Also Like</h2>
              ...
              <div className="space-y-5">
                ...
                <div className="border border-[#7C4DFF]/20 bg-[#7C4DFF]/5 p-5">
                  {entry.potential && (
                    <p className="mb-2 font-display text-xl font-black text-[#7C4DFF]">{entry.potential}</p>
                  )}
                  <a href={ctaUrl} target="_blank" rel="noopener noreferrer"
